...
4. [다운로드 한 데이터셋을 가지고 코드 실행하기 (Colab, RunPods)](#4-다운로드-한-데이터셋을-가지고-코드-실행하기-colab-runpods)
5. [프로젝트 마무리](#5-프로젝트-마무리)
...
## 4. 다운로드 한 데이터셋을 가지고 코드 실행하기 (Colab, RunPods)
...
## **SegFormer를 활용한 전이학습 기반 차선 인식 프로젝트를 끝낸 후 느낀점**

1.  **많은 양의 데이터 수집**: 22분가량의 주행 영상을 수천 장의 이미지로 분할하고, 팀원 각자가 200장씩, 총 1,000장 이상의 이미지를 Roboflow에서 직접 라벨링했습니다. 단순한 클릭이 아니라 폴리건으로 차선 하나하나 신중하게 처리해야 해 고된 작업이었던 것 같습니다.

2.  **여러 환경에서의 시도**: 이 프로젝트를 실행하면서 강의 동안 제일 많이 보고 접했던 RunPod과 Google Colab이라는 전문적인 클라우드 GPU 환경에서 프로젝트를 설정하고 코드를 실행하며 실제 AI 개발 환경에 가까워진 것 같아 뿌듯하고도 자신감이 생긴 것 같습니다.

3.  **최신 모델의 적용**: 단순히 강사님이 주신 기존 코드를 분석하거나 수정하는 것이 아니라, `SegFormer`라는 최신 모델의 구조를 이해하고, 우리가 현재 가지고 있는 데이터에 맞게 전이학습을 적용하여 코드를 만들어내는 경험이 좋았던 것 같습니다.

4. 좋은 모델을 만들어야 하는만큼 ‘좋은 데이터’를 만드는 것이 중요하다는 것을 느꼈던 것 같습니다. 라벨링 규칙을 통일하고, 다양한 환경의 데이터를 확보하는 모든 과정이 최종 결과물에 어떻게 영향을 미치는지 직접 체감할 수 있었던 시간이 된 것 같습니다.

---

- 프로젝트를 진행하며 굉장히 많은 오류가 있었습니다. 알 수 없는 오류부터 시작해, 원인 모를 코드 오류, Jupyter 환경 내 시간 지연 등 굉장히 많은 오류 메시지들을 보면서 claude, chat gpt, ai studio 등을 사용해 구글링하여 코드를 수정하고 검증하는 끈기를 가지게 된 것 같습니다. 확실히 강의에서 개념만 배우는것보다 실제로 스스로 데이터를 찾고, 학습시키며 트러블슈팅을 마주하는 과정이 중요함을 깨달았습니다.
